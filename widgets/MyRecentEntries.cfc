	...
	any function renderIt(
		...
		saveContent variable="rString"{
			...
			for(var x=1; x lte arguments.max; x++){
				writeOutput('<div class="post"><div class="post-content"><a href="#cb.linkEntry(entryResults.entries[ x ])#">#entryResults.entries[ x ].getTitle()#</a><span class="date">#entryResults.entries[ x ].getDisplayPublishedDate()#</span>
</div></div>');
			}
			// close ul
			writeOutput( "</ul>" );
